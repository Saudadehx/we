  ...
  <div class="detail-container" v-if="student">
      ...
      <div class="header-content">
        <div class="header-info">
          ...
          <div class="status-badges" v-if="!isCreatingNew">
            <span class="status-badge" :class="getStatusClass(student.studentStatus)">{{ student.studentStatus }}</span>
            <span class="info-badge" v-if="student.academicYear">第{{ student.academicYear }}学年</span>
            <span class="info-badge" v-if="student.semester">{{ formatSemester(student.semester) }}</span>
          </div>
        </div>
        <div class="header-actions">
          <template v-if="viewMode === 'admin'">
            <button v-if="!isEditing && !isCreatingNew" class="action-btn edit-btn" @click="startEditing">编辑档案</button>
            <button v-if="!isEditing && !isCreatingNew" class="action-btn delete-btn" @click="handleDelete">删除档案</button>
            ...
            <button v-if="isEditing && !isCreatingNew" class="action-btn cancel-btn" @click="cancelEditing">取消编辑</button>
          </template>
    ...
    <div class="content-wrapper">
      <div class="content-grid">
        ...
        <div class="info-card">
          <div class="card-header"><h4>学籍信息</h4></div>
          <div class="card-content"><div class="form-grid">
            <div class="info-item">
            ...
            <div class="info-item"><label class="item-label">当前学年</label><div class="item-content"><span v-if="!isEditing" class="item-value">{{ student.academicYear ? `第 ${student.academicYear} 学年` : '未设置' }}</span><select v-else v-model.number="editableStudent.academicYear" class="form-select" :disabled="viewMode === 'student'"><option :value="null">未设置</option><option v-for="n in 4" :key="n" :value="n">第 {{ n }} 学年</option></select></div></div>
            <div class="info-item"><label class="item-label">当前学期</label><div class="item-content"><span v-if="!isEditing" class="item-value">{{ formatSemester(student.semester) }}</span><select v-else v-model.number="editableStudent.semester" class="form-select" :disabled="viewMode === 'student'"><option :value="null">未设置</option><option value="1">上学期</option><option value="2">下学期</option></select></div></div>
            <div class="info-item"><label class="item-label">学籍状态</label><div class="item-content"><span v-if="!isEditing" class="item-value">{{ student.studentStatus }}</span><select v-else v-model="editableStudent.studentStatus" class="form-select" :disabled="viewMode === 'student'"><option value="在读">在读</option><option value="休学">休学</option><option value="毕业">毕业</option><option value="退学">退学</option></select></div></div>
          </div></div>
        </div>

        <div class="info-card" v-if="isEditing">
          <div class="card-header"><h4>{{ viewMode === 'admin' ? '账户安全' : '修改密码' }}</h4></div>
          <div class="card-content"><div class="form-grid">
            <div class="info-item full-width">
              <label class="item-label">{{ isCreatingNew ? '设置初始密码' : (viewMode === 'admin' ? '重置密码' : '新密码') }}</label>
              <div class="item-content">
                ...
                <p v-else class="field-hint">{{ getPasswordHint() }}</p>
              </div>
            </div>
          </div></div>
        </div>
...
watch(() => props.student, (newStudent) => {
  if (newStudent) {
    ...
    errors.value = {}; // 切换学生时清空错误
  }
}, { immediate: true, deep: true });

// ... 其他辅助函数 getPasswordPlaceholder, formatDate 等保持不变 ...
...
const formatDate = (d) => d ? new Date(d).toLocaleDateString() : '未填写';
...
<style scoped>
/* 之前的样式保持不变，在末尾追加新样式 */
/* ... 省略之前的 .detail-container, .page-header 等样式 ... */
...
<style>
/* 将已有样式粘贴在这里，然后把新增的样式合并或追加进去 */
.detail-container { height: 100%; background: linear-gradient(135deg, var(--color-background) 0%, #f8fafc 50%, var(--color-primary-light) 100%); overflow: hidden; display: flex; flex-direction: column; }
.page-header { background: linear-gradient(135deg, rgba(var(--color-primary-rgb), 0.95), rgba(var(--color-primary-rgb), 0.85)); color: white; padding: var(--spacing-xl); box-shadow: 0 4px 20px rgba(var(--color-primary-rgb), 0.3); }
.header-content { display: flex; justify-content: space-between; align-items: flex-start; gap: var(--spacing-xl); }
.header-info { flex: 1; }
.page-title { font-size: 2.2em; font-weight: 700; margin: 0 0 var(--spacing-sm); letter-spacing: -0.5px; }
.page-subtitle { font-size: 1.1em; margin: 0 0 var(--spacing-md); opacity: 0.9; }
.status-badges { display: flex; gap: var(--spacing-sm); flex-wrap: wrap; }
.status-badge, .info-badge { padding: var(--spacing-xs) var(--spacing-md); border-radius: 20px; font-size: 0.85em; font-weight: 600; background: rgba(255, 255, 255, 0.2); border: 1px solid rgba(255, 255, 255, 0.3); }
...
.header-actions { display: flex; gap: var(--spacing-md); flex-shrink: 0; flex-wrap: wrap; }
.action-btn { display: flex; align-items: center; gap: var(--spacing-sm); padding: 12px 20px; border: none; border-radius: var(--border-radius); font-weight: 600; font-size: 0.9em; cursor: pointer; transition: all var(--transition-speed) ease; white-space: nowrap; border: 1px solid rgba(255, 255, 255, 0.3); }
